...
def skillsRead(skills: dict, debug: bool=False) -> dict:
    ...
    for file in files:
        sub = {}
        treeSkill, rootSkill = Tools.loadXML(path=f'./data/{dir}/{file}', debug=debug)
        for skill in rootSkill.findall('Skill'):
            ...
            skills[skillsClassId] = Tools.sortDictionary(sub, debug)

    print(f"Reading User Skills complete: {skillsNo}")
    return Tools.sortDictionary(skills, debug)
...
def skillsAddData(skills: dict, debug: bool=False) -> dict:
    ...
    for file in files:
        ...
        for skill in rootSkill.findall('String'):
            skillsId = str(skill.get('id'))
            skillsClassId = str(Tools.getClassId(skill.get('class')))
            ...
            if skillsClassId in skills:
                if skillsId in skills[skillsClassId]:
                    ...
                    skills[skillsClassId][skillsId]['race'] = str(Tools.getRaceId(skill.get('race')))
                    skills[skillsClassId][skillsId]['gender'] = str(Tools.getGenderId(skill.get('gender')))
                    ...
                    infoData[f"{infoKey}-race"] = str(Tools.getRaceId(skill.get('race')))
                    infoData[f"{infoKey}-gender"] = str(Tools.getGenderId(skill.get('gender')))
